...
func main() {
	buf := GetBuffer()
	defer buf.Free()
	_ = buf.Write("A Pool is a set of temporary objects that" +
		"may be individually saved and retrieved.")
	_ = buf.Write("A Pool is safe for use by multiple goroutines simultaneously.")
	_ = buf.Write("A Pool must not be copied after first use.")
